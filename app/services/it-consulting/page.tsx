    ...
    <>
      ...
      <style>{`
        ...
        .itc-hero__eyebrow {
          ...
          backdrop-filter:blur(8px);animation:itcFadeUp .9s cubic-bezier(.22,1,.36,1) .1s both;
        }
        ...
        .itc-hero__h1 {
          font-family:'Cormorant Garamond',serif;font-size:clamp(2rem,5vw,3.6rem);
          font-weight:700;line-height:1.1;letter-spacing:-.02em;color:#fff;margin:0 0 1rem;
        ...
        .itc-hero__scroll { position:absolute;bottom:2rem;left:50%;transform:translateX(-50%);z-index:20;display:flex;flex-direction:column;align-items:center;gap:6px;text-decoration:none;animation:itcFadeUp .9s ease .8s both; }
        .itc-hero__scroll-line { width:1px;height:40px;background:linear-gradient(to bottom,rgba(255,255,255,.3),transparent);animation:itcScrollLine 1.8s ease-in-out infinite; }
        @keyframes itcScrollLine { 0%{transform:scaleY(0);transform-origin:top;opacity:1} 50%{transform:scaleY(1);transform-origin:top;opacity:1} 100%{transform:scaleY(1);transform-origin:bottom;opacity:0} }
        .itc-hero__scroll-lbl { font-family:'DM Sans',sans-serif;font-size:9px;font-weight:500;letter-spacing:.2em;text-transform:uppercase;color:rgba(255,255,255,.22); }
        ...
        @media(max-width:768px) {
          .itc-svc__inner--img-left,.itc-svc__inner--img-right { grid-template-columns:1fr;gap:2.5rem; }
          ...
          >
            <div className={`itc-svc__inner--img-${svc.imageLeft ? "left" : "right"}`}>
              ...
              <div className="itc-svc__body" style={{ order: svc.imageLeft ? 2 : 1 }}>
                ...
                <h3 className="itc-svc__heading" id={`itc-svc-heading-${svc.id}`}>
                  {svc.title}
                </h3>
